...
class Pdv extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->permissao = $this->controle_de_acesso_model->verificaPermissao('pdv');
    }
    ...
    public function adicionaProdutoPorCodigoDeBarras($id_caixa)
    {
        ...
        //Separa o código de barras tamanho
        $parsedBarcode = $this->parseBarcode($codigo_de_barras);
        
        $VerificaCodigo =  $parsedBarcode['productCode'];
        $ValorKG        =  $parsedBarcode['weight'];

        $produto = $this->RetornaProduto($VerificaCodigo);

        if(!empty($produto)) :
           if (!$produto['unidade']=='KG'):
            $produto = $this->RetornaProduto($codigo_de_barras);
           endif;
        else :
            $produto = $this->RetornaProduto($codigo_de_barras); 
        endif;
        ...
        if(!empty($produto)) : 
            if ($produto['unidade']=='PD'):
                $valor_unitario = str_replace(",", ".", $quantidade );;
                $quantidade     = 1;
            else:
    ...
    private function parseBarcode($barcode) {
        // Supondo que o código de barras tenha 13 dígitos
        $productCode = substr($barcode, 1, 5); // Primeiros 6 dígitos para o código do produto
        $weight4 = substr($barcode, 6, 4); // Próximos 6 dígitos para o valor
        $weight2 = substr($barcode, 10, 2);
        $weight = $weight4.".".$weight2;
        $checkDigit = substr($barcode, 12, 1); // Últimos 2 dígitos para o dígito verificador
    ...
    private function RetornaProduto($VerificaCodigo)
    {
       $DadosProdutos =  $this->produto_model
        ->select('
            id_produto,
            nome,
            unidade,
        ...
        ')
        ->where('id_empresa', $this->id_empresa)
        ->where('codigo_de_barras', $VerificaCodigo )
        ->first();

        return $DadosProdutos;
    }

    #$barcode = '1234567890123'; // Exemplo de código de barras
    #$parsedBarcode = parseBarcode($barcode);

    #echo "Código do Produto: " . $parsedBarcode['productCode'] . "\n";
    #echo "Peso: " . $parsedBarcode['weight'] . "\n";
    #echo "Dígito Verificador: " . $parsedBarcode['checkDigit'] . "\n";
    ...
    {
        $this->permissaor = $this->controle_de_acesso_model->verificaPermissao('venda_rapida');
        // Verifica se tem permissão de acesso
        if ($url = $this->permissaor):
    ...
    public function alteraQtdDoProduto($id_caixa)
    {
        ...
        $dados['valor_final'] = round((($dados['quantidade'] * $produto['valor_unitario']) - $produto['desconto']),2);
    ...
    public function alteraValorUnitarioDoProduto($id_caixa)
    {
        ...
        $dados[''] = (($produto['quantidade'] * $dados['valor_unitario']) - $produto['desconto']);
    ...
    public function alteraDescontoDoProduto($id_caixa)
    {
        ...
        $dados['valor_final'] = round((($produto['quantidade'] * $produto['valor_unitario']) - $dados['desconto']),2);
    ...
    public function finalizaVenda($id_caixa)
    {
        ...
        foreach ($produtos_do_pdv as $produto) :
            ...
            $subtotal = round($produto['quantidade'] * $produto['valor_unitario'],2);
