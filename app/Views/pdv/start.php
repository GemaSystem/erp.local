...
<body style="background: lightgrey" onafterprint="debug(event)">
            ...
            <div class="col-lg-4">
                <div class="card card-primary">
                    ...
                    <div class="card-footer">
                        <button type="button" name="btnFinalizarVenda" class="btn btn-primary" data-toggle="modal" data-target="#finalizar-venda" <?= (!empty($valor_a_pagar['valor_final'])) ? "" : "disabled" ?>>Finalizar Venda</button>
                    </div>
                </div>
                <!-- /.card -->
            </div>
    ...
    <script>
        document.addEventListener('keydown', function(event) {
            if (event.key === 'F2') {
                event.preventDefault(); // Previne a ação padrão do F1 (abrir ajuda do navegador)
                $('#finalizar-venda').modal('show');
                $(document).ready(function() {
                    $('#forma_de_pagamento').focus();
                });
            }
            if (event.key === 'F1') {
                event.preventDefault(); // Previne a ação padrão do F1 (abrir ajuda do navegador)
